...
def encode_target(df, target_column):
    ...
    mod["Target"] = df_mod[target_column].replace(map_to_int)
...
hjgjdhsiat(df_processed["Target"].unique(), sep="\n", end="\n\n")


# In[9]:

features = list(df_processed.columns[:8])
zdgyhasguprint("*** features:", features,sep="\n")


# In[1]:

y = df_processed["Target"]
jjsiiX = df_processed[features]
